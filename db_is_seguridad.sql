...
-- =============================================================
-- TABLA: personas
-- Identidad base del usuario dentro del sistema
-- El correo_persona es el correo principal de registro y contacto
-- =============================================================
CREATE TABLE personas (
    id_persona UUID PRIMARY KEY DEFAULT gen_random_uuid(),

    nombres_persona VARCHAR(120) NOT NULL,
...
CREATE INDEX idx_personas_estado
ON personas (estado_persona);
...
CREATE TABLE fotos_persona (
    ...
    CONSTRAINT chk_es_principal_foto
        CHECK (es_principal_foto IN (0,1))
);

CREATE INDEX idx_fotos_persona_id_persona
...
-- Separa a la persona de sus metodos de autenticacion
-- No guarda correo para evitar redundancia con personas
-- =============================================================
CREATE TABLE autenticaciones_persona (
    ...
    CONSTRAINT chk_verificado_autenticacion
        CHECK (verificado_autenticacion IN (0,1)),

    CONSTRAINT chk_estado_autenticacion
        CHECK (estado_autenticacion IN (0,1,2,3)),

    CONSTRAINT chk_requiere_cambio_clave_autenticacion
        CHECK (requiere_cambio_clave_autenticacion IN (0,1)),

    CONSTRAINT uq_persona_proveedor_autenticacion
        UNIQUE (id_persona, proveedor_autenticacion),
...
CREATE INDEX idx_autenticaciones_persona_id_persona
ON autenticaciones_persona (id_persona);

CREATE INDEX idx_autenticaciones_persona_proveedor
ON autenticaciones_persona (proveedor_autenticacion);
...
-- OTP asociado al metodo de autenticacion
-- =============================================================
CREATE TABLE codigos_verificacion (
    ...
    CONSTRAINT chk_estado_codigo
        CHECK (estado_codigo IN (0,1,2))
);
...
-- =============================================================
-- TABLA: ingresos
-- Se guarda solo la autenticacion para evitar redundancia
-- =============================================================
CREATE TABLE ingresos (
    ...
    CONSTRAINT chk_resultado_ingreso
        CHECK (resultado_ingreso IN (0,1,2))
);

CREATE INDEX idx_ingresos_id_autenticacion
ON ingresos (id_autenticacion);
...
CREATE TABLE tokens_refresco (
    ...
    CONSTRAINT chk_estado_token
        CHECK (estado_token IN (0,1,2))
);

CREATE INDEX idx_tokens_refresco_id_ingreso
ON tokens_refresco (id_ingreso);

-- =============================================================
-- TABLA: auditoria
-- Tabla fantasma: conserva historial sin FK
-- =============================================================
...
CREATE INDEX idx_auditoria_id_persona
ON auditoria (id_persona);

CREATE INDEX idx_auditoria_id_autenticacion
ON auditoria (id_autenticacion);

CREATE INDEX idx_auditoria_id_ingreso
ON auditoria (id_ingreso);

CREATE INDEX idx_auditoria_fecha
ON auditoria (fecha_auditoria);
