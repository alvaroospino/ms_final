...
import { RegisterPersonaLocalUseCase } from "@/core/application/use-cases/seguridad/register-persona-local.use-case.js";
...
import { RegisterPersonaLocalRequestDto } from "@/modules/seguridad/infrastructure/interface/http/dto/request/register-persona-local.request.dto.js";
...
import { emailConfig, queueConfig } from "@/shared/config/database.config.js";
...
export class PersonaController {
  ...
  private readonly registerPersonaLocalUseCase = new RegisterPersonaLocalUseCase(
    this.personaRepo,
    new PasswordHasher(),
  );
  ...
  private readonly logoutGlobalUseCase = new LogoutGlobalUseCase(
    this.personaRepo,
    this.auditoriaRepo,
  );
  ...
  registerLocal = async (
    ...
    const body = request.body as RegisterPersonaLocalRequestDto;
    const result = await this.registerPersonaLocalUseCase.execute(body);
  ...
  loginLocal = async (
    ...
    return reply.status(200).send({ message: "Inicio de sesión exitoso", data: result });
  };
  ...
  logout = async (
    ...
    return reply.status(200).send({ message: "Sesión cerrada correctamente", data: { success: true } });
  };
  ...
  getMySessionDetail = async (
    ...
    return reply.status(200).send({
      message: "Detalle de sesión consultado correctamente",
      data: {
