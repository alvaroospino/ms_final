...
import { InvalidCredentialsError, InvalidTokenError } from "@/core/application/use-cases/errors/auth-errors.js";
...
export class CambiarClaveUseCase {
  ...
  async execute(input: CambiarClaveInput): Promise<{ success: true }> {
    ...
    if (!auth) throw new InvalidTokenError("Autenticación no encontrada");
